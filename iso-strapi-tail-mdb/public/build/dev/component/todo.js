    ...
    export let todo = {
        views : {
                   'todo-e-1' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `<h1>${_t('todos')} ${todo.name}</h1> <input class="new-todo" data-name="/$todo/name" data-id="todo-myinput" placeholder="What needs to be done?" autofocus>`},
                   'todo-e-2' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `${_t('todos')} ${todo.name}`},
                   'todo-e-4' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `${_t('hier ist nochmal der name')} ${todo.name}`},
                   ...
                   'todo-e-7' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `<input id="toggle-all" data-events="toggle:click" class="toggle-all" type="checkbox" data-name="/$todo/items"> ${_t('Mark all as complete')}`},
                   'todo-e-9' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `<h3>${_t('All')}</h3> <ul class="todo-list" data-name="/$todo/items" data-view="todo-list"> ${_t('...loading...')} </ul>`},
                   'todo-e-10' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `${_t('All')}`},
                ...
                return `<li data-name="/$todo/items/${change.value.id}" data-type="list-item" class="todo-item">
                   ...
                   'todo-e-12' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `<h3>${_t('active')}</h3> <ul class="todo-list" data-name="/$todo/items" data-view="todo-list-active"> </ul>`},
                   'todo-e-13' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `${_t('active')}`},
                   'todo-e-14' : function (args) {
                let { change, todo, _t } = args;
                return !change.value.checked ? this.dom.views["todo-list"].call(this, args) : "";
            },
                   'todo-e-15' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `<h3>${_t('completed')}</h3> <ul class="todo-list" data-name="/$todo/items" data-view="todo-list-completed"> </ul>`},
                   'todo-e-16' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `${_t('completed')}`},
                ...
                return change.value.checked ? this.dom.views["todo-list"].call(this, args) : "";
                   ...
                   'todo-e-19' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `<strong data-name="/$todo/items" data-view="count"></strong>`},
                ...
                switch (args.todo.tab) {
                   ...
                   'todo-e-22' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `<a href="#/all" data-events="tab:click" class="selected">${_t('All')}</a>`},
                   'todo-e-23' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `${_t('All')}`},
                   'todo-e-24' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `<a href="#/active" data-events="tab:click">${_t('Active')}</a>`},
                   'todo-e-25' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `${_t('Active')}`},
                   'todo-e-26' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `<a href="#/completed" data-events="tab:click">${_t('Completed')}</a>`},
                   'todo-e-27' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `${_t('Completed')}`},
                   'todo-e-28' : function (args) { let {change, backendtopmenu,bottommenu,briefing,home,login,modal,signin,tasklist,textlist,todo,topmenu, _t, env} = args; return `${_t('Clear completed')}`},
