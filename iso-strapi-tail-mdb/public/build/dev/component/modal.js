    ...
    export let modal = {
        views : {
                   'modal-e-1' : function (args) { let {change, bottommenu,modal,topmenu, _t, env} = args; return `<!-- Modal content --> <div class="modal-content"> <div class="modal-header"> <h2>${modal.header}</h2> </div> <div class="modal-body"> ${modal.body} </div> <div class="modal-footer"> ${modal.footer} </div> </div> <style> #${modal.name}{ display:${modal.show?'flex':'none'}; } </style>`},
                   'modal-e-2' : function (args) { let {change, bottommenu,modal,topmenu, _t, env} = args; return `<div class="modal-header"> <h2>${modal.header}</h2> </div> <div class="modal-body"> ${modal.body} </div> <div class="modal-footer"> ${modal.footer} </div>`},
                   'modal-e-3' : function (args) { let {change, bottommenu,modal,topmenu, _t, env} = args; return `<h2>${modal.header}</h2>`},
                   'modal-e-4' : function (args) { let {change, bottommenu,modal,topmenu, _t, env} = args; return `${modal.header}`},
                   'modal-e-5' : function (args) { let {change, bottommenu,modal,topmenu, _t, env} = args; return `${modal.body}`},
                   'modal-e-6' : function (args) { let {change, bottommenu,modal,topmenu, _t, env} = args; return `${modal.footer}`},
                   'modal-e-7' : function (args) { let {change, bottommenu,modal,topmenu, _t, env} = args; return `#${modal.name}{ display:${modal.show?'flex':'none'}; }`},
