    ...
    export let modal = {
        views : {
                   'modal-e-1' : function (args) { let {change, backendtopmenu,bottommenu,home,modal,topmenu, _t, env} = args; return `<!-- Modal content --> <div class="modal-content"> <div class="modal-header"> <h2>${modal.header}</h2> </div> <div class="modal-body"> ${modal.body} </div> <div class="modal-footer"> ${modal.footer} </div> </div> <style> #${modal.name}{ display:${modal.show?'flex':'none'}; } </style>`},
                   'modal-e-2' : function (args) { let {change, backendtopmenu,bottommenu,home,modal,topmenu, _t, env} = args; return `<div class="modal-header"> <h2>${modal.header}</h2> </div> <div class="modal-body"> ${modal.body} </div> <div class="modal-footer"> ${modal.footer} </div>`},
                   'modal-e-3' : function (args) { let {change, backendtopmenu,bottommenu,home,modal,topmenu, _t, env} = args; return `<h2>${modal.header}</h2>`},
                   'modal-e-4' : function (args) { let {change, backendtopmenu,bottommenu,home,modal,topmenu, _t, env} = args; return `${modal.header}`},
                   'modal-e-5' : function (args) { let {change, backendtopmenu,bottommenu,home,modal,topmenu, _t, env} = args; return `${modal.body}`},
                   'modal-e-6' : function (args) { let {change, backendtopmenu,bottommenu,home,modal,topmenu, _t, env} = args; return `${modal.footer}`},
                   'modal-e-7' : function (args) { let {change, backendtopmenu,bottommenu,home,modal,topmenu, _t, env} = args; return `#${modal.name}{ display:${modal.show?'flex':'none'}; }`},
                   'modal' : function (args) { let {change, backendtopmenu,bottommenu,home,modal,topmenu, _t, env} = args; return `<div id="${modal.name}" class="modal" data-events="close:click" data-name="/$modal/show" data-id="modal-e-1" data-view="modal-e-1"> <!-- Modal content --> <div class="modal-content" data-id="modal-e-2" data-view="modal-e-2"> <div class="modal-header" data-id="modal-e-3" data-view="modal-e-3"> <h2 data-id="modal-e-4" data-view="modal-e-4" data-name="/$modal/header">${modal.header}</h2> </div> <div class="modal-body" data-id="modal-e-5" data-view="modal-e-5" data-name="/$modal/body"> ${modal.body} </div> <div class="modal-footer" data-id="modal-e-6" data-view="modal-e-6" data-name="/$modal/footer"> ${modal.footer} </div> </div> <style data-id="modal-e-7" data-view="modal-e-7"> #${modal.name}{ display:${modal.show?'flex':'none'}; } </style> </div>`} },
