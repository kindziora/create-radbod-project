    ...
    export let todo = {
       ...
       'todo-e-19' : function (args) { let {change, todo, _t} = args; return `<span class="todo-count" data-name="/$todo/tab" data-events="change" data-view="count"></span> <ul class="filters"> <li> <a href="#/all" data-events="tab:click" class="selected">${_t('All')}</a> </li> <li> <a href="#/active" data-events="tab:click">${_t('Active')}</a> </li> <li> <a href="#/completed" data-events="tab:click">${_t('Completed')}</a> </li> </ul> <button class="clear-completed">${_t('Clear completed')}</button>`},
       ...
       'count' : function(args){
                switch (args.todo.tab) {
                    case "all": return args.todo.items.length;
